...
item = {'id' 'Trial' 'Time' 'Texture' 'Decision' 'GoTime' 'Go' 'No Go' 'Miss' 'False Alarm' 'FAtime'}; %crl added extra heading
decision_log = [decision_log, item];
id = 1;

for i = 1:a
    if strcmp(Event(i),'Begin Trial / Recording') 
        item = {num2str(id) num2str(Trial(i)) '' '' '' '' '' '' '' '' ''}; %% djm added extra field, crl added extra field
        decision_log = [decision_log; item];
        id = id + 1;
...
%cd ..
save (filename, 'decision_log')%, 'Gos', 'NoGos', 'Misses', 'FAs', 'GoTimes', 'FAtimes');
xlswrite([filename '.xls'],decision_log);
